...
export async function analizarResultadoSimulacro({ cargo, preguntas, modelo = 'gemini', simulacro_id, modo }) {
  const headers = await authHeaders()
  const res = await fetch(`${BASE}/api/ia/analisis-resultado`, {
    method: 'POST',
    headers: { ...headers, 'Content-Type': 'application/json' },
    body: JSON.stringify({ cargo, preguntas, modelo, simulacro_id, modo }),
  })
  const json = await res.json()
  if (!res.ok) throw new Error(json.error || 'Error generando análisis.')
  return json.analisis
