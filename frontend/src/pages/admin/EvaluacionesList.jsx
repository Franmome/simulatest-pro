...
export default function EvaluacionesList() {
  ...
  const [stats,        setStats]        = useState({ total: 0, activas: 0, preguntas: 0, niveles: 0 })

  useEffect(() => { cargarEvaluaciones() }, [pagina, busqueda, filtroEstado])
  ...
  async function cargarEvaluaciones() {
    ...
    const conPreguntas = await Promise.all(data.map(async ev => {
      ...
      return { ...ev, totalPreguntas: totalPregs, totalNiveles: levelIds.length }
    }))
  ...
  async function cargarStats() {
    ...
    setStats({ total: total || 0, activas: activas || 0, preguntas: preguntas || 0, niveles: niveles || 0 })
  }
      ...
      <div className="p-8 max-w-7xl mx-auto space-y-8">
        ...
        <div>
          ...
          <p className="text-on-surface-variant mt-1 text-sm">
            Crea y administra los simulacros disponibles en el catálogo.
          </p>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            { label: 'Total evaluaciones', val: stats.total,     icon: 'inventory_2',  color: 'text-primary',   bg: 'bg-primary-fixed'          },
            { label: 'Activas',            val: stats.activas,   icon: 'check_circle', color: 'text-secondary', bg: 'bg-secondary-container/40' },
            { label: 'Total preguntas',    val: stats.preguntas, icon: 'quiz',         color: 'text-tertiary',  bg: 'bg-tertiary-fixed'         },
            { label: 'Niveles creados',    val: stats.niveles,   icon: 'layers',       color: 'text-primary',   bg: 'bg-primary-fixed'          },
          ].map(s => (
            <div key={s.label}
              ...
              <div className="flex gap-1">
                <button
                  ...
                  className="w-8 h-8 rounded-lg flex items-center justify-center text-on-surface-variant
                             hover:bg-surface-container disabled:opacity-30 transition-all"
                >
